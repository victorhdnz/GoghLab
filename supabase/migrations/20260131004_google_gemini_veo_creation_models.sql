-- Inserir modelos Google (Gemini / Veo) para criação: imagem (Nano Banana), vídeo (Veo 3.1), texto (Gemini).
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.creation_ai_models WHERE model_key LIKE 'google/%') THEN
    INSERT INTO public.creation_ai_models (name, logo_url, can_image, can_video, can_prompt, order_position, model_key, credit_cost) VALUES
      ('Gemini Nano Banana (imagem)', NULL, true, false, false, 70, 'google/gemini-2.5-flash-image', NULL),
      ('Gemini Nano Banana Pro (imagem)', NULL, true, false, false, 71, 'google/gemini-3-pro-image-preview', NULL),
