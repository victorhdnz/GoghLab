      ...
      <section id="perfil-marca" className="bg-white rounded-2xl border border-gogh-grayLight p-4 sm:p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-start">
          <div className="flex flex-col">
            ...
            <p className="text-[11px] text-gogh-grayDark mb-1.5 min-h-[2.25rem]">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[11px] text-gogh-grayDark mb-1.5 min-h-[2.25rem]">
              Área de atuação e contexto do negócio; quanto mais específico, melhor a IA personaliza os temas.
            </p>
            <input
              type="text"
          ...
          <div className="md:col-span-2">
            ...
            <p className="text-[11px] text-gogh-grayDark mb-2">
              Selecione em quais formatos de vídeo curto você publica; a criação segue a mesma estrutura (vídeos curtos).
            </p>
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      setProfileForm((f) => {
                        const next = selected
                          ? f.platforms.filter((p) => p !== opt.value)
                          : [...f.platforms, opt.value]
                        return { ...f, platforms: next }
                      })
                    }
                    className={`px-2.5 py-1.5 rounded-md text-xs border ${
                      selected ? 'bg-gogh-yellow/20 border-gogh-yellow text-gogh-black' : 'border-gogh-grayLight text-gogh-grayDark'
                    }`}
                  >
                    {opt.label}
                  </button>
